...
- (void) pinchStarted:(UIPinchGestureRecognizer *)recognizer
{
    ...
    if (abs(_pointOneCellindex - _pointTwoCellindex) == 1)
    {
        ...
        _placeholderCell.frame = CGRectOffset(precedingCell.frame, 0.0f, 60.0f / 2.0f);
...
-(void) pinchChanged:(UIPinchGestureRecognizer *)recognizer
{
    ...
    float cappedGapSize = MIN(gapSize, 60.0f);
    _placeholderCell.transform = CGAffineTransformMakeScale(1.0f, cappedGapSize / 60.0f );
    
    _placeholderCell.label.text = gapSize > 60.0f ?
    @"Release to Add Item" : @"Pull to Add Item";
    
    _placeholderCell.alpha = MIN(0.3f, gapSize / 60.0f);
    
    // determine whether they have pinched far enough
    _pinchExceededRequiredDistance = gapSize > 60.0f;
}
...
-(void) pinchEnded:(UIPinchGestureRecognizer *)recognizer
{
    ...
    {
        // add a new item
        int indexOffset = floor(_tableView.scrollView.contentOffset.y / 60.0f);
